...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    $d = $_POST['date'];
    $query = "INSERT into attendance (adate) values ('$d')";
    $res = mysqli_query($conn,$query);
    foreach($_POST['atnd'] as $usn => $mk)
    {   
        $attendance = $mk['at'];
        // update the row
        $query1 = "UPDATE attendance SET `$usn` = '$attendance' where adate = '$d'";
...
<?php include('sidebar.php'); ?>
    <div class="container">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
           <span>Course: Web Programming</span> </div>
           <div class="adate">
               <p>Enter the date :</p>
    ...
    $res1 = mysqli_query($conn, 'select count(adate) FROM attendance;');
    ...
    if(mysqli_num_rows($result)==0)
    {
      ...
      while($row = mysqli_fetch_array($result)){
          ...
          $res2 = mysqli_query($conn, "select $usn,count($usn) FROM attendance group by $usn having $usn='P'");
